...
namespace WebApiService.Services
{
    ...
    public class JwtService : IJwtService
    {
        ...
        public JwtService(IConfiguration configuration)
        {
            ...
            _issuer = configuration["Jwt:Issuer"] ?? "WebApiService";
        ...
        public string GenerateToken(UserInfo userinfo)
        {
            ...
            var rolesJson = System.Text.Json.JsonSerializer.Serialize(userinfo.scp ?? Array.Empty<string>());

            var claims = new[]
            {
                new Claim(ClaimTypes.NameIdentifier, userinfo.sid.ToString()),
                new Claim(ClaimTypes.Name, userinfo.given_name),
                new Claim(ClaimTypes.Email,userinfo.unique_name),
                new Claim("iss", _issuer),
                new Claim("scp", rolesJson),
                new Claim("family_name",userinfo.family_name),
                new Claim("given_name",userinfo.given_name),
                new Claim("upn",userinfo.upn ?? string.Empty),
                new Claim("tid",userinfo.tid),
                new Claim("sid",userinfo.sid?.ToString() ?? string.Empty)
            };
